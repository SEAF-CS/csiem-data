def import_nasa_modis(CODE_DIR,ACTIONS_DIR,base_path,matlab_data_conversion_data,matlab_data_variable_names,matlab_data_site_coordinates):
    import pandas as pd
    import numpy as np
    import os
    import sys
    ...
    AGENCY_CODE = "NASA"
    PROGRAM = "Moderate Resolution Imaging Spectroradiometer (MODIS)"
    ...
    dir_lst = [
        datapath + "/data-lake/NASA/MODIS/PAR/py_Points",
        datapath + "/data-lake/NASA/MODIS/PAR/py_Polygon",
        datapath + "/data-lake/NASA/MODIS/PIC/py_Points",
        datapath + "/data-lake/NASA/MODIS/PIC/py_Polygon",
        datapath + "/data-lake/NASA/MODIS/POC/py_Points",
        datapath + "/data-lake/NASA/MODIS/POC/py_Polygon"
    ]
    dir_header = [ 
        datapath + "/data-warehouse/csv/nasa/modis/par",
        datapath + "/data-warehouse/csv/nasa/modis/pic",
        datapath + "/data-warehouse/csv/nasa/modis/poc"
    ]
    dir_header_raw = [
        datapath_raw + "/data-warehouse/csv/nasa/modis/par",
        datapath_raw + "/data-warehouse/csv/nasa/modis/pic",
        datapath_raw + "/data-warehouse/csv/nasa/modis/poc"
    ]
    dataset = "NASA"
    ...
    def process_data(dir):
        ...
        for file in os.listdir(dir):
            if file.endswith(".csv") and "Store" not in file:
                ...
                if "MODIS" in dir and "Points" in dir:
                    site = "py_Point_" + file.replace(".csv","").split("_p")[1]
                elif "MODIS" in dir and "py_Polygon" in dir:
        ...
        for file in os.listdir(dir_header):
            if file.endswith('.csv') and file.startswith("py"):
                if "DATA" in file:
                    ...
                    TAG = f'NASA-MODIS-{dir_header.split("/")[-1].upper()}' 
