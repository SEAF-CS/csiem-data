def import_moi_nemo(CODE_DIR,ACTIONS_DIR,base_path,matlab_data_conversion_data,matlab_data_variable_names,matlab_data_site_coordinates):
    import pandas as pd
    import numpy as np
    import os
    import sys
    ...
    AGENCY_CODE = "MOI"
    PROGRAM = "Nucleus for European Modelling of the Ocean (NEMO)"
    ...
    dir_lst = [
        datapath + "/data-lake/MOI/NEMO/model_salinity/Points",
        datapath + "/data-lake/MOI/NEMO/model_salinity/Polygon"
    ]
    dir_header = [ 
        datapath + "/data-warehouse/csv/moi/nemo/model_salinity",
        datapath + "/data-warehouse/csv/moi/nemo/model_salinity"
    ]
    dir_header_raw = [
        datapath_raw + "/data-warehouse/csv/moi/nemo/model_salinity",
        datapath_raw + "/data-warehouse/csv/moi/nemo/model_salinity"
    ]
    dataset = "MOI"
        ...
        for file in os.listdir(dir_header):
            if file.endswith('.csv') and file.startswith("py") and not file.startswith("._"):
                if "DATA" in file:
                    ...
                    TAG = f'MOI-NEMO-{dir_header.split("/")[-1].upper()}' 
