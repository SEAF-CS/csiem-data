def import_moi_pisces(CODE_DIR,ACTIONS_DIR,base_path,matlab_data_conversion_data,matlab_data_variable_names,matlab_data_site_coordinates):
    import pandas as pd
    import numpy as np
    import os
    import sys
    ...
    AGENCY_CODE = "MOI"
    PROGRAM = "Pelagic Interactions Scheme for Carbon and Ecosystem Studies (PISCES)"
    ...
    dir_header = [ 
        ...
        datapath + "/data-warehouse/csv/moi/pisces/model_car",
        datapath + "/data-warehouse/csv/moi/pisces/model_co2",
        datapath + "/data-warehouse/csv/moi/pisces/model_nut",
        datapath + "/data-warehouse/csv/moi/pisces/model_optics",
        datapath + "/data-warehouse/csv/moi/pisces/model_pft",
    ]
    dir_header_raw = [
        datapath_raw + "/data-warehouse/csv/moi/pisces/model_bio",
        datapath_raw + "/data-warehouse/csv/moi/pisces/model_car",
        datapath_raw + "/data-warehouse/csv/moi/pisces/model_co2",
        datapath_raw + "/data-warehouse/csv/moi/pisces/model_nut",
        datapath_raw + "/data-warehouse/csv/moi/pisces/model_optics",
        datapath_raw + "/data-warehouse/csv/moi/pisces/model_pft",
    ]
    dataset = "MOI"
    moi_data = get_conversion_data(dataset,matlab_data_conversion_data)
        ...
        for file in os.listdir(dir):
            ...
            for variable in variables:
                ...
                for field in moi_data.dtype.names:
                    old_name = moi_data[field][0, 0]['Old'][0, 0][0]
                    if old_name == variable:
                        conv_factor = float(moi_data[field][0, 0]['Conv'][0, 0][0])
                        Id = moi_data[field][0, 0]['ID'][0, 0][0]
                        break
        ...
        for file in os.listdir(dir_header):
            if file.endswith('.csv') and file.startswith("py") and not file.startswith("._"):
                if "DATA" in file:
                    ...
                    TAG = f'MOI-PISCES-{dir_header.split("/")[-1].upper()}' 
                    if "MODEL_BIO" in TAG:
