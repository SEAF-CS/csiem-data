def import_esa_globcolor(CODE_DIR,ACTIONS_DIR,base_path,matlab_data_conversion_data,matlab_data_variable_names,matlab_data_site_coordinates):
    import pandas as pd
    import numpy as np
    import os
    import sys
    ...
    AGENCY_CODE = "ESA"
    PROGRAM = "Global Ocean Colour (GLOBCOLOR)"
    ...
    dir_lst = [
        datapath + "/data-lake/ESA/GC/Optics/Point",
        datapath + "/data-lake/ESA/GC/Optics/Polygon",
        datapath + "/data-lake/ESA/GC/Plankton/Points",
        datapath + "/data-lake/ESA/GC/Plankton/Polygon",
        datapath + "/data-lake/ESA/GC/PP/Point",
        datapath + "/data-lake/ESA/GC/PP/Polygon",
        datapath + "/data-lake/ESA/GC/Reflectance/Point",
        datapath + "/data-lake/ESA/GC/Reflectance/Polygon",
        datapath + "/data-lake/ESA/GC/Transp/Point",
        datapath + "/data-lake/ESA/GC/Transp/Polygon"
    ]
    dir_header = [ 
        datapath + "/data-warehouse/csv/esa/gc/optics",
        datapath + "/data-warehouse/csv/esa/gc/plankton",
        datapath + "/data-warehouse/csv/esa/gc/pp",
        datapath + "/data-warehouse/csv/esa/gc/reflectance",
        datapath + "/data-warehouse/csv/esa/gc/transp"
    ]
    dir_header_raw = [
        datapath_raw + "/data-warehouse/csv/esa/gc/optics",
        datapath_raw + "/data-warehouse/csv/esa/gc/plankton",
        datapath_raw + "/data-warehouse/csv/esa/gc/pp",
        datapath_raw + "/data-warehouse/csv/esa/gc/reflectance",
        datapath_raw + "/data-warehouse/csv/esa/gc/transp"
    ]
        ...
        for file in os.listdir(dir_header):
            if file.endswith('.csv') and file.startswith("py") and not file.startswith("._"):
                if "DATA" in file:
                    ...
                    TAG = f'ESA-GC-{dir_header.split("/")[-1].upper()}' 
                    if "OPTICS" in TAG:
                        TAG = TAG.replace("OPTICS","OPT")
                    elif "PLANKTON" in TAG:
                        TAG = TAG.replace("PLANKTON","PLK")
                    elif "PP" in TAG:
                        TAG = TAG.replace("PP","PP")
                    elif "REFLECTANCE" in TAG:
                        TAG = TAG.replace("REFLECTANCE","REF")
                    elif "TRANSP" in TAG:
                        TAG = TAG.replace("TRANSP","TRA")
