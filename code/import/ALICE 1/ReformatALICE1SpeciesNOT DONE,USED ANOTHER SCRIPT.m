...
%function ALICESpecies()
    ...
    outdir = ['ReformatedAlice1Species.csv'];
    ...
    ReformatedTable = table();

    ReformatedTable.Date(1)=datetime;
    ReformatedTable.Site(1)={'fakesitename'};
    ReformatedTable.DepthBool(1)={'B'};
    ReformatedTable.SpeciesName(1)={'fakeSpeciesname'};
    ReformatedTable.GroupName(1)={'fakeGroupname'};
    ReformatedTable.Concentration(1)=100;
        ...
        for col = 11:16 %hardcoded to match file 
            ...
            for SpeciesNum = 1:height(Surface)
                ...
                if SURFBOOL
                    ...
                    %add Surf row
                    ReformatedTable.Date(Count)=Date;
                    ReformatedTable.Site(Count)={Site};
                    ReformatedTable.DepthBool(Count)={Depth};
                    ReformatedTable.SpeciesName(Count)={varname};
                    ReformatedTable.GroupName(Count)={'fakeGroupname'};
                    ReformatedTable.Concentration(Count)=DataVal;
