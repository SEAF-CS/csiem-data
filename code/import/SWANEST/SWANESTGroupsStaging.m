function SWANESTGroupsStaging()

    run('../../actions/csiem_data_paths.m')

    main_file = [datapath,'data-lake/SWANEST/ALL PROGRAMS - PHYTO AWARE_2012-2020.xlsx'];
    outdir = [datapath,'data-warehouse/csv_holding/swanest/Groups/'];
    ...
    VarListStruct = agency.SWANESTGroups;
    ...
    for row = 1:height(DataTable)
        varname =  DataTable{row,14}; % col 14
        [AgencyStruct,AgencyIndex] = SearchVarlist(VarListStruct,varname);
        if AgencyIndex == 0 
            fprintf(unimprtedFID,"%s\n",varname{1});
            continue
        ...
        if ~exist(fDATA,'file')
            ...
            fid = fopen(fHEADER,'w');
                ...
                fprintf(fid,'Tag,SG-E-SWANEST_Plankton_Groups\n');
