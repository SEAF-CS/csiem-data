...
for i = 1:length(sites)
    ...
    for j = 1:length(vars)
        ...
        theheader = ['Agency Name,Agency Code,Program,Project,Station Status,Lat,Long,Time Zone,Vertical Datum,',...
            'National Station ID,Site Description,Data Classification,Variable,Date,Depth,Data,QC'];
        ...
        for k = 1:length(swan.(sites{i}).(vars{j}).Date)
            ...
            fprintf(fid,'%s,%s,%s,%s,%s,%6.6f,%6.6f,%s,%s,%s,%s,%s,%s,',...
                'Department of Water and Environmental Regulation','DWER','Estuary','SWANEST','Active',swan.(sites{i}).(vars{j}).Lat,swan.(sites{i}).(vars{j}).Lon,...
                'GMT +8','mAHD',regexprep(sites{i},'s',''),regexprep(sites{i},'s',''),class,var_full);
