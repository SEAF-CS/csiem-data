...
for i = 46:length(dirlist)%3
    ...
    if strcmpi(filelist(1).name,'WaterQualityDiscreteForSiteCrossTab.xlsx')
        filename = ['../../../data-lake/dwer/swanest/',dirlist(i).name,'/WaterQualityDiscreteForSiteCrossTab.xlsx'];
        type = 'WQ';
%         if i ~= 9
         [rows,cols] = calculate_xls_size(filename);
%         end
    else
        filename = ['../../../data-lake/dwer/swanest/',dirlist(i).name,'/WaterLevelsContinuousForSiteCrossTab.xlsx'];
        type = 'Level';
