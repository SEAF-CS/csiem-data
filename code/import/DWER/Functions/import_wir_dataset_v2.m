...
switch type
    ...
    case 'Level'
        ...
        swan_add = format_level_data_1(data,headers,ver);
...
[sNum,sStr] = xlsread('WIR Supporting Data.xlsx','Site_Information','A2:H1000000');
...
function swan_add = format_level_data_1(data,headers,ver)
...
%         nDate(k) = datenum([TDate{1},' ', newswan.(names{i}).Time{k}],'dd/mm/yyyy HH:MM:SS');
        ...
        nDate(k) = datenum(newswan.(names{i}).Date(k),'dd/mm/yyyy');
...
u_sites(l)
                ...
                if ~isempty(ww)
                    ...
                    if isfield(newswan.(names{i}),'Sample_Depths_M_1')
                        ...
                        swan_add.(Shortname{tt(1)}).(vars{j}).Depth(1:length(swan_add.(Shortname{tt(1)}).(vars{j}).Date),1) = {0};
                    end
                end
            end
        end
...
for i = 1:length(names)
    ...
    newswan.(names{i}).Site_ID;
    
    
    if ver == 2
        for iki = 1:length(newswan.(names{i}).Site_ID)
            if ischar(newswan.(names{i}).Site_ID{iki})
                sites(iki,1) = str2num(newswan.(names{i}).Site_ID{iki});
            else
                sites(iki,1) = newswan.(names{i}).Site_ID{iki};
            end
        end
    else
        sites = cell2mat(newswan.(names{i}).Site_ID);
    ...
    for j = 1:length(vars)
        ...
        if ~isempty(ss)
            if strcmp(new_name{ss(1)},'Ignore') == 0
                ...
                if isfield(swan2.(sites{i}).(vars{j}),'QC')
                    qc = swan2.(sites{i}).(vars{j}).QC;
                    
                else
                    qc(1:length(tdepth),1) = {'N'};
                end
                ...
                clear tdata tdata_1 tdate tdepth qc
            end
...
%     end
%     
%     if ~isfield(swan.(sites{i}),'WQ_TRC_SS1') & isfield(swan.(sites{i}),'WQ_NCS_SS1')
% 
...
for i = 1:length(sites)
    ...
    for j = 1:length(vars)
        ...
        mQC = fdata.(sites{i}).(vars{j}).QC;
        mDC = fdata.(sites{i}).(vars{j}).QC;
        ss = find(~isnan(mdata));
        disp([sites{i},': ',vars{j}]);
        if ~isempty(ss)
