function import_phytoplankton_Species(WCNum,SheetString,DataLastRowNum)
    ...
    VarListStruct = agency.(['WCWA',num2str(WCNum),'_PhytoplanktonSpecies']);
    SiteListStruct = sitekey.WCWA3Phyto;
    %% all Sheets that this code imports have same sites as the first sheet
    varnameInsideAgencyStruct = fieldnames(VarListStruct);

    unimprtedFID = fopen(['UnimportedSpecies',num2str(WCNum),'.txt'],"w");
