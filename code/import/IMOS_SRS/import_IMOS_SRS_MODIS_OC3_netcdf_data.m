...
ncname = "D:\csiem\data-lake\IMOS\srs\MODIS\Chla\IMOS_aggregation_20231218T010807Z.nc";

outdir = 'D:\csiem\data-warehouse\csv\imos\srs_oc3\';mkdir(outdir);
    ...
    for j = 1:length(data.longitude)
        ...
        if inpolygon(data.longitude(j),data.latitude(i),shp(1).X,shp(1).Y)
            ...
            if ~isempty(sss)
                ...
                thetag = ['IMSO-',upper('SRS-MODIS-OC3')];
                fprintf(fid,'Tag,%s\n',thetag);
                fprintf(fid,'Data File Name,%s\n',regexprep(datafile,outdir,''));
                fprintf(fid,'Location,%s\n',['data-warehouse/csv/imos/',lower('SRS_MODIS_OC3')]);
