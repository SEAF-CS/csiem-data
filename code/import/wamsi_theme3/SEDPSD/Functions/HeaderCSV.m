function HeaderCSV(InputFlatFile,VarKey)
    ...
    for VarNum = 1:length(UniqueVars)
        ...
        for SiteNum = 1:length(UniqueSites)
            ...
            fileName = ['/Projects2/csiem-data-hub/data-warehouse/csv/wamsi/wwmsp3.1_SEDPSD/',char(UniqueSites(SiteNum)) ,'_',char(UniqueVars(VarNum))];
    ...
    for DataNum = 1:height(Table)
        ...
        fileName = ['/Projects2/csiem-data-hub/data-warehouse/csv/wamsi/wwmsp3.1_SEDPSD/',char(SiteName) ,'_',char(VarName)];
                fileName = filenameGoodifier(fileName);
