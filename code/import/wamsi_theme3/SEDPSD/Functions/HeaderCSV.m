function HeaderCSV(InputFlatFile,VarKey)
    ...
    for VarNum = 1:length(UniqueVars)
        ...
        for SiteNum = 1:length(UniqueSites)
            ...
            fileName = ['../../../../../data-warehouse/csv/wamsi/wwmsp3.1_SEDPSD/',char(UniqueSites(SiteNum)) ,'_',char(UniqueVars(VarNum))];
    ...
    for DataNum = 1:height(Table)
        ...
        fileName = ['../../../../../data-warehouse/csv/wamsi/wwmsp3.1_SEDPSD/',char(SiteName) ,'_',char(VarName)];
                fileName = filenameGoodifier(fileName);
