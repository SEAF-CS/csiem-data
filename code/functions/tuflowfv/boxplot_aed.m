...
axislims = reconcileLimits(ax,axislims);
axis(ax,axislims);
...
if fullfactors==true
    ...
    groupIndexByPoint = (gMidx-1).*colweight;
...
function axislims = reconcileLimits(ax,axislims)
% With "hold on" allow limits to expand but not shrink
if ~isempty(ax) && isequal(ax.NextPlot,'add')
    axislims(1) = min(axislims(1),ax.XLim(1));
    axislims(2) = max(axislims(2),ax.XLim(2));
    axislims(3) = min(axislims(3),ax.YLim(1));
    axislims(4) = max(axislims(4),ax.YLim(2));
end
end
