...
if strcmpi(varname{1},'H') == 0
    data1.surface = savedata.(varname{1}).Top;
    if isfield(savedata.(varname{1}),'Bot')
        data1.bottom = savedata.(varname{1}).Bot;
    else
        data1.bottom = savedata.(varname{1}).Top;
    end
    
else
    data1.surface = savedata.(varname{1});
    data1.bottom = savedata.(varname{1});
end
...
[data.surface,c_units,isConv] = tfv_Unit_Conversion_pf(data1.surface,varname{1});
[data.bottom,c_units,isConv]  = tfv_Unit_Conversion_pf(data1.bottom,varname{1});
