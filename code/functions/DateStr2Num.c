...
// Author: Jan Simon, Heidelberg, (C) 2010-2012 matlab.THISYEAR(a)nMINUSsimon.de
//
// See also DATESTR, DATENUM, DATEVEC.
// FEX: DateConvert 25594 (Jan Simon)

/*
% $JRev: R-i V:009 Sum:NPGqk8w59Rli Date:28-Jul-2013 23:32:25 $
...
double Str32Num(const mxArray *S);
...
static int32_T cumdays[] = {0, 0,31,32,59,90,120,151,181,212,243,273,304,334};

// Standard method to add leap day:
#define ADD_LEAP_DAY \
  if (mon > 2) { \
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[])
{
   ...
   if (nrhs == 1) {
      ...
      switch (Format) {
		 ...
		 case   32:  CoreFcn = Str32Num;    break;
...
double Str0Num(const mxArray *S)
{
  ...
  if (mxGetNumberOfElements(S) != 20) {
     mexErrMsgIdAndTxt(ERR_ID   "BadDateString",
                       ERR_HEAD "Bad size for [dd-mmm-yyyy HH:MM:SS] format.");
  }
  ...
  if (mIndex <= 25955) {  // Split the test in 2 halfs for speed
     switch (mIndex) {
        ...
        default:
          mexErrMsgIdAndTxt(ERR_ID "BadDateString",
                       ERR_HEAD "Bad month for [dd-mmm-yyyy HH:MM:SS] format.");
     }
  } else {
     switch (mIndex) {
...
double Str1Num(const mxArray *S)
{
  ...
  if (mxGetNumberOfElements(S) != 11) {
     mexErrMsgIdAndTxt(ERR_ID   "BadDateString",
                       ERR_HEAD "Bad size for [dd-mmm-yyyy] format.");
  }
  ...
  if (mIndex <= 25955) {  // Split the test in 2 halfs for speed
     switch (mIndex) {
        ...
        default:
          mexErrMsgIdAndTxt(ERR_ID   "BadDateString",
                            ERR_HEAD "Bad month for [dd-mmm-yyyy] format.");
     }
  } else {
     switch (mIndex) {
        case 25968:  mon = 9;   break;  // 'sep'
...
double Str29Num(const mxArray *S)
{
  ...
  if (mxGetNumberOfElements(S) != 10) {
      mexErrMsgIdAndTxt(ERR_ID   "BadDateString",
                        ERR_HEAD "Bad size for [yyyy-mm-dd] format.");
  }
...
double Str30Num(const mxArray *S)
{
  ...
  if (mxGetNumberOfElements(S) != 15) {
      mexErrMsgIdAndTxt(ERR_ID   "BadDateString",
                        ERR_HEAD "Bad size for [yyyymmddTHHMMSS] format.");
  }
...
{
  // Convert a single string to a serial date number.
  // "dd/mm/yyyy HH:MM"  "01/01/2000 12:15"
  ...
  // Check number of characters:
  if (mxGetNumberOfElements(S) != 16) {
      mexErrMsgIdAndTxt(ERR_ID   "BadDateString",
                        ERR_HEAD "Bad size for [yyyy-mm-dd HH:MM:SS] format.");
  }
  ...
  year = (int32_T) ((d16[6] - 48) * 1000 + (d16[7] - 48) * 100 +
          d16[8] * 10 + d16[9]  - 528);
  mon  = d16[3]  * 10 + d16[4]  - 528;  // (d16[5]-48) * 10 + d16[6]-48
  day  = d16[0]  * 10 + d16[1]  - 528;
...
{
  // Convert a single string to a serial date number.
  // "HH:MM:SS dd/mm/yyyy"  "12:15:00 01/01/2000"
  ...
  // Extract the date and time:
  d16  = (uint16_T *) mxGetData(S);     // mxChar is UINT16!
  year = (int32_T) ((d16[15] - 48) * 1000 + (d16[16] - 48) * 100 +
          d16[17] * 10 + d16[18]  - 528);
  mon  = d16[13]  * 10 + d16[14]  - 528;  // (d16[5]-48) * 10 + d16[6]-48
  day  = d16[10]  * 10 + d16[11]  - 528;
  hour = d16[00] * 10 + d16[01] - 528;
  min  = d16[4] * 10 + d16[5] - 528;
  sec  = d16[7] * 10 + d16[8] - 528;;
...
double Str1000Num(const mxArray *S)
{
  ...
  if (mxGetNumberOfElements(S) != 24) {
     mexErrMsgIdAndTxt(ERR_ID "BadDateString",
                    ERR_HEAD "Bad size for [dd-mmm-yyyy HH:MM:SS.FFF] format.");
  }
  ...
  if (mIndex <= 25955) {  // Split the test in 2 halfs for speed
     switch (mIndex) {
        ...
        default:
          mexErrMsgIdAndTxt(ERR_ID "BadDateString",
                        ERR_HEAD "Bad month in [dd-mmm-yyyy HH:MM:SS] format.");
     }
  } else {
     switch (mIndex) {
        case 25968:  mon = 9;   break;  // 'sep'
...
double Str1030Num(const mxArray *S)
{
  ...
  if (mxGetNumberOfElements(S) != 19) {
      mexErrMsgIdAndTxt(ERR_ID   "BadDateString",
                        ERR_HEAD "Bad size for [yyyymmddTHHMMSS.FFF] format.");
  }
